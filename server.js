...
async function waitForWorkflowCompletion(token, owner, repoName, maxWaitMinutes = 20) {
    console.log(`⏳ Waiting for GitHub Actions to build fresh executables...`);
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (latestRun) {
                console.log(`🔄 Attempt ${attempt}/${maxAttempts}: Workflow status = ${latestRun.status}`);
                
                if (latestRun.status === 'completed') {
                    if (latestRun.conclusion === 'success') {
                        console.log(`✅ Build completed successfully! Getting fresh download URLs...`);
                        ...
                        console.log(`🎉 Fresh download URLs obtained:`, downloadUrls);
                        return downloadUrls;
